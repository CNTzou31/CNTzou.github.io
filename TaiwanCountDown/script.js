        ...
        .then(cloudData => {
            ...
            for (const key in cloudData) {
                ...
                if (!isNaN(dateObj.getTime())) {
                    // Successfully parsed as a date, convert to YYYY-MM-DD
                    const normalizedKey = dateObj.toISOString().split('T')[0];
                    normalizedCloudData[normalizedKey] = cloudData[key];
                } else {
                    // Key is already in the correct format or not a date
                    normalizedCloudData[key] = cloudData[key];
                }
            }
            console.log("Cloud data (NORMALIZED):", normalizedCloudData);
            ...
            const mergedData = { ...localData, ...normalizedCloudData };
