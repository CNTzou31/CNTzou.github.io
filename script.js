document.addEventListener('DOMContentLoaded', () => {
    // --- State ---
    let students = JSON.parse(localStorage.getItem('bingoStudents')) || [];
    let savedPassword = localStorage.getItem('bingoAdminPwd') || "admin";
    const BINGO_SIZE = 5;
    ...
    // --- Logic ---
    function saveState() {
        localStorage.setItem('bingoStudents', JSON.stringify(students));
    }
    ...
    async function changePassword() {
        ...
        if (current === savedPassword) {
            ...
            if (newPwd && newPwd.trim().length > 0) {
                ...
                localStorage.setItem('bingoAdminPwd', savedPassword);
